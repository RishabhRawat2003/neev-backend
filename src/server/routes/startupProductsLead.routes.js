...
router.route('/new').post(async (req, res) => {
    try {
       if (!_.isEmpty(req.body)) {
            const outputResult = await addNewStartupProductsLeadHandler(req.body.startupProductsLead);
            res.status(responseStatus.STATUS_SUCCESS_OK);
            res.send({
                status: responseData.SUCCESS,
                data: {
...
router.route('/:id/update').post( async (req, res) => {
    try {
        if (!_.isEmpty(req.params.id) && !_.isEmpty(req.body) && !_.isEmpty(req.body.startupProductsLead)) {
            let input = {
                objectId: req.params.id,
                updateObject: req.body.startupProductsLead
            }
